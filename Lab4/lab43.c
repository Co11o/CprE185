...
-----------------------------------------------------------------------------*/
	...
	int directionLR(double gx, double gy, double gz, double value);
	int directionUD (double gx, double gy, double gz, double value);
	int directionTF (double gx, double gy, double gz, double value);
...
*/
    ...
    scanf("%d, %lf, %lf, %lf, %lf, %lf, %lf, %d, %d, %d, %d", &t, &ax, &ay, &az, &gx, &gy, &gz, &b1, &b2, &b3, &b4 );
	 ...
	 if(directionLR(gx, gy, gz, value)==1&& (value != 1)){
	printf("Left\n");
	value = 1;
	}		
	else if (directionLR(gx, gy, gz, value)==2 && (value != 2)) {
		printf("Right\n");
		value = 2;
	}
	if (directionUD(gx, gy, gz, value)==3 && (value != 3)) {
		printf("Up\n");
		value =3;
	}
	else if (directionUD(gx, gy, gz, value)==4 && (value != 4)) {
		printf("Down\n");
		value= 4;
	}
	if (directionTF(gx, gy, gz, value)==5 && (value != 5)) {
		printf("Away\n");
		value = 5;
	}
	else if (directionTF(gx, gy, gz, value)==6 && (value != 6)) {
		printf("Towards\n");
		value = 6;
	}
...
return 0;
		...
		else if((close_to (tolerancePass, pointP, gx) == 1) ){
		
		return 1;
	}
	    else if((close_to (tolerancePass2, pointN, gx) == 1) ){
...
return 0;
		...
		else if((close_to (tolerancePass, pointP, gy) == 1) ){
		
		
		return 3;
	}
	    else if((close_to (tolerancePass, pointN, gy) == 1)){
...
return 0;
		...
		else if((close_to (tolerancePass2, pointP, gz) == 1) ){
		
		
		return  5;
	}
	    else if((close_to (tolerancePass, pointN, gz) == 1)){
