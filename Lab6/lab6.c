...
int read_acc(double* a_x, double* a_y, double* a_z, int* time, int* Button_UP, int* Button_DOWN, int* b_Square, int* Button_RIGHT);
...
int main()
{
	...
	do
	{
		read_acc(&x ,&y,&z,&t,&b_X,&b_Triangle,&b_Square,&b_Circle);	
	 	// Get line of input
		...
		fflush(stdout);
	} while (b_Square==0); // Modify to stop when left button is pressed
	return 0;
}
int read_acc(double* a_x, double* a_y, double* a_z, int* time, int* Button_UP, int* Button_DOWN, int* b_Square, int* Button_RIGHT) {
    int b5=0;
