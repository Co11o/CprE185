...
int main()
{
	...
	{
		// Get line of input
 read_line(double* g_x, double* g_y, double* g_z, int* time, int* Button_T, int* Button_X, int* Button_S, int* Button_C);
 ...
 scaleRadsForScreen(rad);
		// Output your graph line
 //graph_line(int number);
...
int read_line(double* g_x, double* g_y, double* g_z, int* time, int* Button_T, int* Button_X, int* Button_S, int* Button_C){
scanf("%d,%lf,%lf,%lf,%d,%d,%d,%d,%d,%d",time,g_x, g_y,g_z, Button_T,Button_X,Button_S,Button_C);
	if(*Button_T==1) {
		return 1;
	}
	else {
